...
const flatten = (elements) => {
  ...
  const arr = [];
  for (let i = 0; i < elements.length; i++) {
    const arrChecker = elements[i];
    if (Array.isArray(arrChecker) === false) arr.push(elements[i]);
    else {
      const nestedArr = flatten(elements[i]);
      for (let j = 0; j < nestedArr.length; j++) arr.push(nestedArr[j]);
    }
  }
  return arr;
};
